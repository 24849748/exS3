...
void button_init(button_event_cb_t short_cb, button_event_cb_t long_cb){
    ...
    btn.isPressing = false;
    btn.previousSteadyState = gpio_get_level(ECD_PIN_BTN);
    btn.lastSteadyState = gpio_get_level(ECD_PIN_BTN);
    btn.lastFlickerableState = gpio_get_level(ECD_PIN_BTN);
